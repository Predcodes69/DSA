...
set<vi> s;
vi temp;
void gen(vi &v, vi &temp, int i, set<vi> &s, int &k, int curr){
    if(i == v.size())
    {
        if(accumulate(all(temp), 0LL) == k)
            s.emplace(temp);
        return;
    }
    if(curr >= v[i])
    {
        temp.eb(v[i]);
        gen(v, temp, i, s, k, curr - v[i]);  //picking the same element again 
        temp.ppb();//backtracking
    }
    gen(v, temp, i + 1, s, k, curr);    //not picking the current element and moving on to the next index
}
/*----------------------------------------------------------------------MAIN CODE------------------------------------------------------------------------------------------------------------------------*/
void solve()
{
