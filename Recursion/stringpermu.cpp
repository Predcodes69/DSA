...
string temp;
vi hsh(27, 0);
void permu(vector<string> &ans, string &s, string temp, vi &hsh)
{
...
In other words, return true if one of s1's permutations is the substring of s2.*/
    ...
    for(auto x : ans)
        m[x]++;
    string curr = "";
    rep(i, 0, s2.length() - s1.length() + 1)
    {
        curr = s2.substr(i, s1.length());
        if(m[curr])
        {
            yes;
            return;
