...
void solve()
{
  //every even integer can be expressed as a sum of two primes
    //Noldbach problem states that at 
    // least k prime numbers from 2 to n inclusively can be expressed as the sum of three integer numbers: two neighboring prime numbers and 1
    int n, k;
    cin >> n  >> k;
    int ct = 0;
    //check if k prime numbers between 2 to n satisfy the criteria: sum of 1 + (2 neighboring primes)
    debug(primenums);
    rep(i, 1, primenums.size())
    {
        //check sum of 2 neighboring primes
        ct+=(primes[primenums[i - 1] + primenums[i] + 1] and (primenums[i - 1] + primenums[i] + 1 <= n));
    }
    if(ct >= k)
    yes;
    else
