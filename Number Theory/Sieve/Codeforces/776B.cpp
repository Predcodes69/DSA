...
void solve()
{
  int k;
  cin >> k;
  //paint differently if the price of the item is the prime divisor of the another color
  //paint all prime prices as 1 and all composite numbers as 2, since no composite number can be a prime divisor of any other number itself.
  vi prices;
  int f = 0;
  rep(i, 1, k + 1)
  {
    if(primes[i + 1] == i + 1)
       ...
       f = 1,  prices.eb(2);   //composite price
  }
  int ans = f?2:1;
  cout << min((int)(prices.size()), (int)(ans)) << endl;
  print(prices, prices.size());
}
