...
void solve()
{
    //how to fucking check the number is divisible by both of the numbers p and q? hOw
  int n;
  cin >> n;
  vi v(n);
  read(v, n);
  vi hsh(N, 0);
  for(auto x : v)
    hsh[x]++; //counting the counts of the number in the array
  for(int i = 1; i <= N; i++)
  {
    for(int j = i; j <= N; j+=i)
    {
