...
//         if (primes[i] == 1)
//         {
//             primes[i] = i;  //marking the minimum factors of the prime numbers as the prime number itself.
//             for (int j = i * i; j <= N; j += i)
//             {
//                 if (!primes[j])
//                     primes[j] = i; // storing min primes, and marking the other composite numbers with their minimum prime factors.
//             }
//         }
//     }
// }
vi primes(N, 1);//marking all the numbers as primes initially.
void sieve()
{
    primes[0] = primes[1] = 0;//since these numbers are exceptions.
    for(int i = 2; i*i<=N; i++)
    {
        if(primes[i])
        for(int j = i*i; j <= N; j+=i)
