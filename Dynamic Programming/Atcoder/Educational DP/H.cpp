...
void solve()
{
    ...
    vector<string> s(h);
    read(s, h);
    //the base case, the only way to be at the first s[0][0] is 1, since we are starting from the position, and any position less than that is 0 
    vvi dp(h, vi(w, 0));
    dp[0][0] = 1;
    rep(i, 0, h)
    {
        rep(j, 0, w)
        {
            //the only way to reach (i, j) is from (i - 1, j) and (i, j - 1) while checking the value of i and j if they are greater than 0 and also that they do not have mines
            if(s[i][j] == '.')  //empty
            {
                if(i > 0)
                    (dp[i][j]+=dp[i - 1][j])%=M;//coming from the top coordinate(moving down)
                if(j > 0)
                    (dp[i][j]+=dp[i][j - 1])%=M;//coming from the left coordinate(moving right)
            }
            else{
                dp[i][j] = 0;//wall
            }
