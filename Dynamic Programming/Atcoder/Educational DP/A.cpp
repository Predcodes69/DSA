...
void solve()
{
    ...
    vi v(n);
    read(v, n);
    //initially, the frog is on v[0], so no jump was done, and hence no cost, so our base case is dp[0] = 0;
    //our state can be dp[i] where dp[i] = minimum possible cost to reach stone numbered i having height v[i];
    //our possible ways to reach i is from i - 1 and i - 2
    //final subproblem is dp[n].
    vi dp(n + 2, INT_MAX);
    dp[0] = 0;
    dp[1] = abs(v[1] - v[0]);    //only way being to reach stone numbered 1.
    rep(i, 2, n)
    {
