...
void sieve()
{
    ...
    vi dp(1e6 + 10, INT_MAX);    //initially marking all the combinations impossible.
/*----------------------------------------------------------------------MAIN CODE------------------------------------------------------------------------------------------------------------------------*/
void solve()
{
    int n, target;
    ...
    read(v, n);
    sort(all(v));
    //the minimum most coin's value can be achieved in only one way
    dp[0] = 1;//same concept as the last sum(task 1633)
    // rep(i, 1, x + 1)
    // {
    //     for(auto x : v)
    //     {
    //         if(i - x >= 0)
    //         {
    //             //lets go on to minimise the number of coins required to make any intermediate sum possible(subtask)
    //             dp[i] = min(dp[i - x])
    //         }
    //     }
    // }
