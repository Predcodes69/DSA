...
void solve()
{
    // to solve a dp problem, always solve intermediate subtasks of a dp problem.
    //to reach a sum i(i < n), there are only six ways to reach i, since there are numbers 1 to 6 in a dice, 
    //that is we can reach the sum i from i - 1, i - 2, upto i - 6.
    //therefore dp[i] would be the summation of the ways we can reach the sum i from i - 6 to i - 1
    //we will initialize dp[0] = 1, and not 0 since there is only one way we can reach the sum 0, i.e an empty set
    /*DONT confuse dp[0] = 0, since we are not asked about the number of coins to make the sum 0, we are asked about the number of ways, and in the number of ways,
    we construct a power set which also includes the null set => {}, therefore dp[0] = 1;*/
    int n;
    cin >> n;
    dp[0] = 1;
    rep(i, 1, n + 1)
