...
void sieve()
{
    ...
    vi dp(1e6 + 10, INT_MAX);
/*----------------------------------------------------------------------MAIN CODE------------------------------------------------------------------------------------------------------------------------*/
void solve()
{
    int n;
    cin >> n;
    //our subproblem can be number of steps to make i --> 0, that is , dp[i] represents the minimum number of steps to make i ==> 0
    //and at each step, we have choices equal to the number of distinct digits in the number, and the base case id dp[0] = 0, and make sure the digit to be subtracted is not 0
    dp[0] = 0;
    rep(i, 1, n + 1)
    {
